...
class CROMA(nn.Module):
    ...
    def forward(self, imgs, radar_mask_info, optical_mask_info, rank, world_size):
        ...
        radar_encodings = self.radar_encoder(
            imgs=radar_imgs, attn_bias=radar_masked_attn_bias, mask_info=radar_mask_info)
        optical_encodings = self.optical_encoder(
            imgs=optical_imgs, attn_bias=optical_masked_attn_bias, mask_info=optical_mask_info)
...
class Attention(nn.Module):
    ...
    def forward(self, x, alibi):
        ...
        q, k, v = map(lambda t: rearrange(
            t, 'b n (h d) -> b h n d', h=self.attention_heads), (q, k, v))
...
def apply_mask_to_alibi(alibi, ids_keep_queries, ids_keep_keys, batch_size, orig_seq_len, masked_seq_len,
    ...
    alibi_long_sequence = rearrange(alibi.repeat(
        batch_size, 1, 1, 1), 'b n i j -> b (i j) n')
...
def gather_features(features, world_size):
    gathered_image_features = [torch.zeros_like(
        features) for _ in range(world_size)]
...
class ContrastLossInput(nn.Module):
    ...
    def forward(self, radar_features, optical_features, world_size, rank):
        ...
        radar_features = radar_features / \
            radar_features.norm(dim=1, keepdim=True)
        optical_features = optical_features / \
            optical_features.norm(dim=1, keepdim=True)

        # gather features from other GPUs
        all_radar_features = gather_features(
            features=radar_features, world_size=world_size)
        all_optical_features = gather_features(
            features=optical_features, world_size=world_size)
        ...
        labels = torch.arange(
            num_logits, device=radar_features.device, dtype=torch.long)
        labels = labels + num_logits * rank

        # calculate loss
        loss = (F.cross_entropy(logits_per_optical, labels) +
                F.cross_entropy(logits_per_radar, labels)) / 2
        return loss
...
class ViT(nn.Module):
    ...
    def forward(self, imgs, attn_bias, mask_info=None):
        ...
        if mask_info is None:
            ...
            x_masked = apply_mask_to_sequence(
                x=x, ids_keep=mask_info['ids_keep'])
...
class DecoderMAE(nn.Module):
    def __init__(self,
        ...
        self.decoder_pos_embed = nn.Parameter(torch.zeros(
            1, num_patches, self.decoder_dim), requires_grad=False)
        decoder_pos_embed = get_2d_sincos_pos_embed(
            self.decoder_pos_embed.shape[-1], int(num_patches ** .5), cls_token=False)
    ...
    def forward(self, x, mask_info_radar, mask_info_optical, target):
        ...
        mask_tokens = self.mask_token.repeat(
            x.shape[0], mask_info_radar['ids_restore'].shape[1] + 1 - x.shape[1], 1)
        x = torch.cat([x, mask_tokens], dim=1)
        x = torch.gather(
            x, dim=1, index=mask_info_radar['ids_restore'].unsqueeze(-1).repeat(1, 1, x.shape[2]))
        ...
        pred = rearrange(x, 'b (h w) (c i j) -> b c (h i) (w j)',
                         c=14, i=8, j=8, h=15, w=15)
        pred_optical = rearrange(
            pred[:, :12, :, :], 'b c (h i) (w j) -> b (h w) (c i j)', c=12, i=8, j=8)
        pred_radar = rearrange(
            pred[:, 12:, :, :], 'b c (h i) (w j) -> b (h w) (c i j)', c=2, i=8, j=8)
        ...
        target = rearrange(
            target, 'b (h w) (c i j) -> b c (h i) (w j)', c=14, i=8, j=8, h=15, w=15)
        target_optical = rearrange(
            target[:, :12, :, :], 'b c (h i) (w j) -> b (h w) (c i j)', c=12, i=8, j=8)
        target_radar = rearrange(
            target[:, 12:, :, :], 'b c (h i) (w j) -> b (h w) (c i j)', c=2, i=8, j=8)
        ...
        loss_optical = (loss_optical * mask_info_optical['mask_for_mae']).sum(
            # mean loss on removed patches
        ) / mask_info_optical['mask_for_mae'].sum()
        ...
        loss_radar = (loss_radar * mask_info_radar['mask_for_mae']).sum(
            # mean loss on removed patches
        ) / mask_info_radar['mask_for_mae'].sum()
