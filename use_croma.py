...
class PretrainedCROMA(nn.Module):
    def __init__(self, pretrained_path='CROMA_base.pt', size='base', modality='both', image_resolution=120):
        ...
        # check types
        assert type(
            pretrained_path) == str, f'pretrained_path must be a string, not {type(pretrained_path)}'
        assert type(size) == str, f'size must be a string, not {type(size)}'
        assert type(
            modality) == str, f'modality must be a string, not {type(modality)}'
        assert type(
            image_resolution) == int, f'image_resolution must be an int, not {type(image_resolution)}'
        ...
        assert modality in [
            'both', 'SAR', 'optical'], f'modality must be either both, SAR, or optical, not {modality}'
        ...
        if modality in ['SAR', 'both']:
            ...
            self.s1_encoder = ViT(dim=self.encoder_dim, depth=int(
                self.encoder_depth/2), in_channels=self.s1_channels)
            ...
            self.s1_encoder.load_state_dict(
                checkpoint['s1_encoder'])
            self.GAP_FFN_s1.load_state_dict(
                checkpoint['s1_GAP_FFN'])
        ...
        if modality in ['optical', 'both']:
            ...
            self.s2_encoder.load_state_dict(
                checkpoint['s2_encoder'])
            self.GAP_FFN_s2.load_state_dict(
                checkpoint['s2_GAP_FFN'])
    ...
    def forward(self, SAR_images=None, optical_images=None):
        ...
        if self.modality in ['SAR', 'both']:
            ...
            SAR_GAP = self.GAP_FFN_s1(
                SAR_encodings.mean(dim=1))  # (bsz, encoder_dim)
        ...
        if self.modality in ['optical', 'both']:
            ...
            optical_GAP = self.GAP_FFN_s2(
                optical_encodings.mean(dim=1))  # (bsz, encoder_dim)
            return_dict['optical_encodings'] = optical_encodings
            return_dict['optical_GAP'] = optical_GAP

        if self.modality == 'both':
            joint_encodings = self.cross_encoder(x=SAR_encodings, context=optical_encodings, relative_position_bias=self.attn_bias.to(
                optical_images.device))  # (bsz, num_patches, encoder_dim)
...
class Attention(nn.Module):
    ...
    def forward(self, x, relative_position_bias):
        ...
        q, k, v = map(lambda t: rearrange(
            t, 'b n (h d) -> b h n d', h=self.num_heads), (q, k, v))
